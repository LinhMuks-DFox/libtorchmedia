...
namespace audio4torch::io {
    ...
    auto inline load_audio(const std::string &path) -> load_audio_t {
        ...
        torch::Tensor tensor = torch::from_blob(samples.data(), {static_cast<int64_t>(samples_read)}, options).clone();
        ...
        if (num_channels > 1) {
            tensor = tensor
            .view({static_cast<int64_t>(num_channels), static_cast<int64_t>(samples_read / num_channels)})
            .contiguous();
        }
    ...
    auto inline save_audio(torch::Tensor audio_tensor, const std::string &path, int sample_rate, int sample_depth) -> bool {
        // Initialize SoX library
        if (sox_init() != SOX_SUCCESS) {
            throw std::runtime_error("Failed to initialize SoX");
        }
