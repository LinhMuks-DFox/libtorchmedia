...
auto _apply_convolve_mode(torch::Tensor conv_result, int64_t x_length, int64_t y_length, convolve_mode mode) -> torch::Tensor
{
    switch (mode) {
    case full:
        return conv_result;
    case valid: {
        auto target_length = std::max(x_length, y_length) - std::min(x_length, y_length) + 1;
    ...
    default:
        throw std::invalid_argument("Unrecognized mode value. Please specify one of full, valid, same.");
    }
}

auto convolve(tensor_t x, tensor_t y, convolve_mode mode) -> tensor_t
{
    /*
    ...
    */
    ...
    auto num_signals = torch::tensor(x.sizes().slice(0, -1)).prod().item<int64_t>();
...
auto amplitude_to_DB(tensor_t signal, float amin, float db_multiplier, float topdb, bool apply_topdb) -> tensor_t
{
    ...
    // Apply logarithm
    tensor_t db = 10.0 * torch::log10(torch::clamp(power, amin, std::numeric_limits<float>::max()));
...
auto spectrogram(tensor_t signal, spectrogram_option option) -> tensor_t
{
    ...
    tensor_t window = option._window.defined() ? option._window : torch::hann_window(win_length, torch::TensorOptions().dtype(signal.dtype()));
